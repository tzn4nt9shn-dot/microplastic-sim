...
st.set_page_config(page_title="해류 이용 미세플라스틱 포집 시뮬레이터", page_icon="🌊", layout="wide")

st.title("🌊 GIS 기반 무동력 미세플라스틱 포집 시뮬레이터 (v2.2 - 수중 물리 모델 적용)")
...
def run_simulation_engine(u_x, u_y, angle_deg, spec, size_mm, n_particles):
    ...
    # 2. 여유 부력 비율 및 슬라이딩 이탈 확률 산출
    reserve_buoyancy = (RHO_WATER - spec["density"]) / RHO_WATER
    submergence_factor = np.clip(reserve_buoyancy / 0.1366, 0.3, 1.0) # PP=1.0, LDPE=0.82, HDPE=0.47
    
    rad = np.radians(angle_deg / 2.0)
    angle_efficiency = np.cos(rad) # 30도=0.966, 90도=0.707 (각도가 좁을수록 유실 적음)
    ...
    # 입자별 물리적 유실 마스크 (부력 및 와류에 의해 펜스 아래로 누수 판정)
    ...
    for step in range(max_steps):
        ...
        for i in range(n_particles):
            ...
            if px >= wall_x_threshold and px <= apex_x + 1.0:
                if is_underflow_escaped[i]:  # 부력 부족으로 펜스 밑으로 통과한 입자
                    v_eff = u_ocean
                else:
...
with tab1:
    ...
    with col_vis:
        ...
        if run_btn:
            ...
            if is_sunk:
                st.error("⚠️ [PET 침강 경고] PET는 밀도가 해수보다 높아서 수심 0.5m 펜스 아래로 침강합니다. (포집율: 0.0%)")
            else:
                total_steps = len(trajs)
                frame_stride = max(1, total_steps // 40)
        ...
        for reg_name, (ux, uy) in [("GPGP", (0.15, -0.05)), ("SouthSea", (0.25, 0.10)), ("Kuroshio", (0.75, 0.35))]:
            for ang in [30, 45, 60, 90]:
                for p_name, p_spec in PLASTIC_SPECS.items():
                    _, r, _, _, sunk = run_simulation_engine(ux, uy, ang, p_spec, 0.5, 200)
                    res_list.append({
                        "Region": reg_name,
                        "Angle": f"{ang}°",
